...
api-key endpoints are gone. Preference is just the default TanoAudit tier.
"""
...
async def test_model_settings_default_tier_roundtrip(auth):
    ...
    r = await client.put(
        ...
        headers=headers,
        json={"default_tier": "tanoaudit_deep"},
    )
    assert r.status_code == 200
    assert r.json()["data"]["default_tier"] == "tanoaudit_deep"
