# Agent prompts — Akira AI frontend wiring (parallel-safe)
...
- The frontend is `frontend/`, no build step (React 18 + Babel Standalone, plain
  ...
  already uses it.
- **All backend calls go through `window.AkiraAPI`** (defined in `js/api.js`). Responses
  are already unwrapped (the `{data,error}` envelope is handled); failures throw
  `AkiraAPI.ApiError` with `.code`, `.message`, `.status`. **Never call `fetch` directly.
  Never edit `js/api.js`.** If a method you need is missing from `AkiraAPI`, STOP and
  leave a `// TODO(slice0): AkiraAPI.<ns>.<method>` comment instead of adding it — the
...
- Backend runs at `http://localhost:8000`, frontend served at `http://localhost:8765`
  ...
  `verify@akira.dev` / `hunter2pass` — or register via the UI.
...
> (use `window.AkiraAPI`, never edit `api.js`/`app.jsx`/`index.html`).
>
> Wire each section to its endpoints:
> - **Profile** (`ProfileSec`): load `AkiraAPI.profile.get()` (← `GET /profile`), save via
>   `AkiraAPI.profile.update(patch)` (← `PATCH /profile`). Replace any `VS_REPO_META` use.
> - **Account / password** (`AccountSec`): `AkiraAPI.security.changePassword(...)`
...
>   `AkiraAPI.security.*` (`/security/2fa/*`, `/security/sessions`, `/security/login-history`).
> - **API keys** (`ApiKeysSec`): `AkiraAPI.settings.getApiKeys/putApiKeys/testApiKey/
>   deleteApiKey` (`/settings/api-keys*`).
> - **Models** (`ModelsSec`): `AkiraAPI.settings.getModels/putModels` (`/settings/models`).
> - **Privacy** (`PrivacySec`): `AkiraAPI.settings.getPrivacy/putPrivacy` (`/settings/privacy`).
> - **Usage** (`UsageSec`): `AkiraAPI.usage.get()` (`/usage`).
> - **Notifications** (`NotifSec`): `AkiraAPI.notifications.getPreferences/putPreferences`
>   (`/notifications/preferences`).
> - **Handoff links** (`HandoffLinksSec`): `AkiraAPI.handoff.links()` +
>   `AkiraAPI.handoff.deleteLink(id)` (`/handoff-links`).
>
> Some `AkiraAPI` methods above may not exist yet (Slice 0 is adding them in parallel).
...
> add/remove an API key. Report which `AkiraAPI` methods you needed that were missing.
...
> with `AkiraAPI.learning.categories()` (`GET /learning-hub/categories`),
...
> - status: `AkiraAPI.github.status()` (`GET /github/status`)
> - connect: redirect the browser to `AkiraAPI.github.authorizeUrl()` (`GET /github/authorize`
>   returns/links the OAuth URL — follow how the backend returns it)
> - disconnect: `AkiraAPI.github.disconnect()`
> - repos: `AkiraAPI.github.repos()` (`GET /github/repos`)
> - trigger / issue / status-check / repo-access settings → the corresponding
>   `AkiraAPI.github.*` PATCH methods (`/github/triggers`, `/issue-settings`, etc.)
> - webhook deliveries: `AkiraAPI.github.deliveries()`
>
...
> error message).
>
> Missing `AkiraAPI` methods → `// TODO(slice0): ...`. Add loading/error/empty states.
>
...
> `ReportsPage` uses `AkiraAPI.reports.*` — see WIRING.md.)
>
> - **Custom Vulnerabilities** (`CustomVulnsPage`): CRUD via `AkiraAPI.customVulns.*`
>   (`/custom-vulnerabilities`), and "research" → `AkiraAPI.customVulns.research(...)`
>   (`POST /custom-vulnerabilities/research`). Replace `VS_CUSTOM_VULNS`.
> - **Optimization Plans** (`PlansPage`): CRUD + goals + validate via `AkiraAPI.plans.*`
>   (`/optimization-plans`, `/optimization-plans/{id}/goals`, `/optimization-plans/validate`).
>   The repo picker uses `AkiraAPI.watchlist.repositories({github_only:true})`
>   (`GET /watchlist/repositories?github_only=true`) — show an empty-state if no GitHub
>   connection. Replace `VS_PLANS`.
> - **Watchlist** (`WatchlistPage`): `AkiraAPI.watchlist.list()`, `.repositories()`,
>   `.pin(id)/.unpin(id)`, `.frequency(id, freq)`, `.alerts()`, `.rescan(id)`
>   (`/watchlist*`). Replace `VS_WATCHLIST`.
> - **Reports** (`ReportsPage`): list/download exports + shares via `AkiraAPI.reports.*`
>   (per-scan: `/scans/{id}/exports`, `/scans/{id}/share`; download via
>   `AkiraAPI.reports.downloadExportUrl(reportId)`). If this needs a scan context the page
...
>   `AkiraAPI.github.repos()` (`GET /github/repos`). If GitHub isn't connected, the call
...
>   `AkiraAPI.settings.getModels()` (`GET /settings/models`). Confirm the returned shape
...
> `AkiraAPI.scans.list()` (`GET /scans`) and `AkiraAPI.usage.get()` (`GET /usage`); derive
